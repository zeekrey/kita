...
async function seed() {
	...
	// Insert groups
	const [sonnenkaefer, schmetterlinge, loewenzahn] = await db
		.insert(gruppen)
		.values([
	...
	await db.insert(kinder).values([
		...
		{ vorname: 'Mia', nachname: 'Weber', geburtstag: '2021-07-22', gruppeId: sonnenkaefer.id },
		{
			vorname: 'Ben',
			nachname: 'Fischer',
			geburtstag: '2020-11-08',
			gruppeId: schmetterlinge.id
		},
		{
			vorname: 'Hannah',
			nachname: 'Meyer',
			geburtstag: '2021-02-14',
			gruppeId: schmetterlinge.id
		},
		{
			vorname: 'Paul',
			nachname: 'Wagner',
			geburtstag: '2020-09-30',
			gruppeId: schmetterlinge.id
		},
	...
	// Insert teachers
	const [maria, thomas, lisa, anna] = await db
		.insert(erzieher)
		.values([
			...
			mealEntries.push(
				{
					datum: dayStr,
					typ: 'fruehstueck',
					beschreibung: getMealForDay(day.getDay(), 'fruehstueck')
				},
				{
					datum: dayStr,
					typ: 'mittagessen',
					beschreibung: getMealForDay(day.getDay(), 'mittagessen')
				},
	...
	await db.insert(ankuendigungen).values([
		{
			titel: 'Sommerfest',
			nachricht:
				'Am 15. Juli findet unser jährliches Sommerfest statt. Alle Familien sind herzlich eingeladen! Bitte bringt gute Laune und einen Salat oder Kuchen mit.',
			gueltigVon: todayStr,
		...
		},
		{
			...
			nachricht:
				'Ab dem 1. Februar öffnen wir bereits um 7:00 Uhr statt 7:30 Uhr. Die Schließzeit bleibt bei 18:00 Uhr.',
			gueltigVon: todayStr,
			gueltigBis: formatDate(nextWeek),
			prioritaet: 'normal'
		},
		{
			...
			nachricht:
				'Das Wetter wird wechselhaft. Bitte stellen Sie sicher, dass Ihr Kind Wechselkleidung in der Kita hat.',
			gueltigVon: todayStr,
			gueltigBis: formatDate(nextWeek),
			prioritaet: 'normal'
		}
