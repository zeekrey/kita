	...
	<div class="flex h-screen bg-gray-100">
		...
		<aside class="w-64 bg-white shadow-md">
			<div class="p-4 border-b">
				<h1 class="text-xl font-bold text-gray-800" style="font-family: var(--font-heading);">Kita Admin</h1>
			</div>
			<nav class="p-4">
				<ul class="space-y-2">
					{#each navItems as item}
						<li>
							<a
								href={item.href}
								class="flex items-center gap-3 px-4 py-2 rounded-lg transition-colors {isActive(item.href)
